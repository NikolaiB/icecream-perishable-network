...
function gpsReading(gpsReading) {
    ...
    var SF_TERMINAL = '/LAT:37.7749N/LONG:122.4194W';
    ...
    if (latLong == SF_TERMINAL) {
        var shipmentInPortEvent = factory.newEvent(NS, 'ShipmentTerminalEvent');
        shipmentInPortEvent.shipment = shipment;
        var message = 'Shipment has reached the terminal of ' + SF_TERMINAL;
...
function setupDemo(setupDemo) {
    ...
    var manufacturer = factory.newResource(NS, 'Manufacturer', 'manufacturer@email.com');
    ...
    // create the retailer
    var retailer = factory.newResource(NS, 'Retailer', 'retailer@email.com');
    var retailerAddress = factory.newConcept(NS, 'Address');
    retailerAddress.country = 'USA';
    ...
    contract.minTemperature = -30; // min temperature for the cargo
    contract.maxTemperature = -18; // max temperature for the cargo
    contract.minPenaltyFactor = 0.2; // we reduce the price by 20 cents for every degree below the min temp
    contract.maxPenaltyFactor = 0.1; // we reduce the price by 10 cents for every degree above the max temp

    // create the shipment
    var shipment = factory.newResource(NS, 'Shipment', 'SHIP_001');
    shipment.type = 'ICE_CREAM';
